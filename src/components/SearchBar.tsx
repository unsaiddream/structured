...
function SnippetWithHighlight({ text, terms }: { text: string; terms: string[] }) {
    ...
    <>
      {parts.map((part, i) =>
        regex.test(part) ? (
          <mark
            key={i}
            className="bg-yellow-200 text-yellow-900 rounded-sm px-0.5 font-medium not-italic"
          >
            {part}
          </mark>
        ) : (
...
export default function SearchBar({ onResults, onClear }: Props) {
  ...
  // Group hits by syllabusId for display
                ...
                return (
                  <div key={syllabusId} className="border-b border-gray-100/70 last:border-0">
                    ...
                    {syllabusHits.map((hit) => {
                      const idx = globalHitIdx++
                      const isSelected = idx === selectedIdx
                      return (
                        <button
                          key={`${hit.syllabusId}-${hit.section}-${hit.title}-${idx}`}
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-sm font-semibold text-gray-900 truncate">
                                <SnippetWithHighlight text={hit.title} terms={hit.terms} />
                              </span>
                              <span
                            ...
                            <p className="text-xs text-gray-500 leading-relaxed line-clamp-2">
                              <SnippetWithHighlight text={hit.snippet} terms={hit.terms} />
                            </p>
                          </div>
                          <ArrowRight
